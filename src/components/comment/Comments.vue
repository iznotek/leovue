    ...
    <div v-if="!loading" class="innerWrapper">
      <div class="addComment" key="addComment">
        ...
        <div class="commentBox" key="commentBox" v-show="!showSignPanel && !auth || auth ">
            ...
            ></textarea>
            <button :style="{backgroundColor: themeColor}" aria-label="Send" v-if="requestLoading"><div class="requestLoading"></div></button>
            <button :style="{backgroundColor: themeColor}" @click="addComment" aria-label="Send" v-else>Send</button>
