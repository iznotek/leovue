...
AFRAME.registerComponent('camera-transform-controls', {
  ...
  onButtonDown: function onButtonDown(evt) {
    var left
    var target
    ...
    this.panningController.entity.object3D.getWorldPosition(this.panningController.dragStartPoint)
    this.released = this.isLeftButtonDown && this.isRightButtonDown
  },
  onButtonUp: function onButtonUp(evt) {
    var left
    var target
    target = evt.target
  ...
  twoHandInteraction: function () {
    ...
    return function () {
      ...
      if (this.released) {
        ...
        this.initAngle = this.prevAngle = Math.atan2(currentPositionLeft.x - currentPositionRight.x, currentPositionLeft.z - currentPositionRight.z)
        midPoint.copy(currentPositionLeft).add(currentPositionRight).multiplyScalar(0.5)
        this.prevDistance = prevDistanceVec3.copy(currentPositionLeft).sub(currentPositionRight).length()
        this.released = false
      }

      currentDistance = currentDistanceVec3.copy(currentPositionLeft).sub(currentPositionRight).length()
      deltaDistance = this.prevDistance - currentDistance //Get center point using local positions.

      centerVec3.copy(this.leftHandEl.object3D.position).add(this.rightHandEl.object3D.position).multiplyScalar(0.5) // Set camera rig scale.
      ...
      translation = centerVec3.applyQuaternion(this.cameraRigEl.object3D.quaternion).multiplyScalar(deltaDistance)
      this.cameraRigEl.object3D.position.sub(translation)
      this.cameraRigEl.setAttribute('position', this.cameraRigEl.object3D.position) // Set camera rig rotation.

      currentAngle = Math.atan2(currentPositionLeft.x - currentPositionRight.x, currentPositionLeft.z - currentPositionRight.z)
  ...
  rotateScene: function () {
    ...
    return function (midPoint, deltaAngle) {
      var cameraRigEl = this.cameraRigEl
      var rotation // Rotate the direction.

      dirVec3.copy(cameraRigEl.object3D.position).sub(midPoint).applyAxisAngle(UP, -deltaAngle)
  ...
  processPanning: function () {
    ...
    return function () {
      ...
      deltaPosition.copy(dragStartPoint).sub(currentPosition) // Apply panning.
  ...
  registerHand: function registerHand(entity, hand) {
